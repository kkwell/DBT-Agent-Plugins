...
- `dbt_start_flash_image` / `dbtflashstart` plus `dbt_get_job_status` / `dbtjobstatus`
  ...
  - direct `.img` burning uses the same job path with `scope=raw` and
    `host_image_file=/path/to/image.img`; the runtime detects Rockchip
    RKFW/RKAF update packages and flashes their partition image set internally.
    Only true raw disk images are written from LBA 0.
