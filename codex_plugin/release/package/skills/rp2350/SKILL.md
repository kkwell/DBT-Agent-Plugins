...
- For direct live-board status requests such as `开发板状态` or `当前开发板状态`, call `dbt_current_board_status` immediately as the first action; do not run `dbtctl status`, shell commands, web search, workspace probes, or repository/skill-file reads before that MCP tool.
