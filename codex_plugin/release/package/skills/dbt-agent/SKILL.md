...
- For TaishanPi `rgb_led` requests that only set a color, turn the LED off, use heartbeat, or use a simple timer blink, call `dbt_apply_effect` directly. Do not generate C and do not call `dbt_build_run_program` for those simple LED effects.
- For TaishanPi code-generation and deployment tasks that really need generated C/C++ source, fetch capability context and board config first, then use `dbt_build_run_program`.
