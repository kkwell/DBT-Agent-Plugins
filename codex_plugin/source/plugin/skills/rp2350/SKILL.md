...
- For macOS GUI requests such as `安装 GUI`, `安装 mac 端 GUI`, `安装图形界面`, `卸载 GUI`, or `uninstall GUI`, call `dbt_install_gui` or `dbt_uninstall_gui` directly. These tools only manage the optional `DBT-Agent.app`; they must not remove the shared runtime, dbt-agentd, board environments, Codex plugin, or OpenCode plugin.
