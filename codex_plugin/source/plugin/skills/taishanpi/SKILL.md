...
- For Qt or QtQuick application requests, do not route through `linux_program`, do not look for `qmlscene`, and do not hand-assemble compiler/linker commands. After `dbt_get_board_config(probe_env=true)`, create the Qt project in the current user workspace and call `dbt_qt_build_run_app` with `project_dir` and `workspace`.
- QtQuick apps should be compiled executables, usually with CMake and `qt_add_qml_module`; the board image may only contain the Qt runtime under `/userdata/qt6-rk3566-llvm`, not developer launchers such as `qmlscene`.
- User Qt projects deploy to `/userdata/qt-project/<ProjectName>` by default. Use `qpa_platform` only for board display backend options such as linuxfb rotation; app-level layout or rotation should normally be implemented in the generated QML/C++ source.
- For board-app autostart requests, call `dbt_configure_autostart` after deployment. Use `app_type=qt` with `remote_runner_path` from `dbt_qt_build_run_app`, and use `app_type=native` with `remote_binary_path`/`remote_artifact_path` from `dbt_build_run_program`. Do not inspect systemd/SysV/OpenRC, do not create per-app systemd units, and do not write custom `/etc/init.d` app scripts; DBT owns the fixed `/userdata/.dbt-autostart` broker and single boot hook.
